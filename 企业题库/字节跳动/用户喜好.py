...
# 超时
# n = int(input())
# hobby = list(map(int, input().split()))
# q = int(input())
# find = list()
# temp = list()
# for _ in range(q):
...
#   high = find[i][1] - 1
#   while low <= high:
#       if low == high and hobby[low] == find[i][-1]:
#         res += 1
#       else:
#         if hobby[low] == find[i][-1]: 
#           res += 1
#         if hobby[high] == find[i][-1]:
#           res += 1
#       high -= 1
#       low += 1
#   print(res)
...
for i in range(q):
  ...
  right = find[i][-2] - 1
  while left <= right:
    if left == right and hobby[left] == find[i][-1]:
      res += 1
      continue
    if hobby[left] == find[i][-1]:
      res += 1
    if hobby[right] == find[i][-1]:
      res += 1
    left += 1
    right -= 1
  print(res)