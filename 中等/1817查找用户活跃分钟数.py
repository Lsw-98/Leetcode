...
def findingUsersActiveMinutes(logs, k):
    ...
    while True:
        ...
        temp = logs[j][0]
        while logs[j][0] == temp:
            if logs[j] not in minute:
                minute.append(logs[j])
            j += 1
            if j == len(logs):
                break
        if len(minute) > 0:
...
print(findingUsersActiveMinutes([[283268890,14532],[283268891,14530],[283268889,14530],[283268892,14530],[283268890,14531]], 2))
